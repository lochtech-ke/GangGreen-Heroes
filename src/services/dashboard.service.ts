    ...
    try {
      ...
      for (const forest of forests) {
        ...
        const { data: initiatives, error: initError } = await supabase
          .from('initiatives')
          .select('id, trees_planted, area_hectares')
          .eq('forest', forest);
    ...
    try {
      ...
      transactions?.forEach(txn => {
        activities.push({
          ...
          description: `${txn.quantity_tons} tons from ${txn.carbon_credit?.initiative?.title || 'Unknown'}`,
          timestamp: txn.transaction_date,
          forest: txn.carbon_credit?.initiative?.forest
        });
      });
