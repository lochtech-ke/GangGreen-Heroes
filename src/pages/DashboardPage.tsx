...
import { 
  Search, Leaf, TrendingUp, Users, Trophy, 
  MapPin, Camera, Award
} from 'lucide-react';
...
export function DashboardPage() {
  ...
  const loadDashboardData = async () => {
    try {
      setLoading(true);
      const [metricsData, forestData, activityData] = await Promise.all([
        dashboardService.getImpactMetrics('all'),
        dashboardService.getForestStats(),
        dashboardService.getActivityFeed(5),
      ]);
...
function InitiativeCard({ forestStats, loading }: any) {
